...
function addToDict(button) {
    ...
    todoDict[`${parentSpanID}taskname`] = addedtask.value;
    console.log(todoDict);
}

function removeFromDict(button) {
    var parentSpan = button.closest('span');
    if (parentSpan) {
        var parentSpanID = parentSpan.id;
    }
